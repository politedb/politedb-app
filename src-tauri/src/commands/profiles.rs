...
fn profile_dir(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    std::fs::create_dir_all(&dir).map_err(|e| format!("CREATE_PROFILE_DIR_FAILED: {e}"))?;
...
#[tauri::command]
pub async fn profile_list(app: AppHandle) -> Result<Vec<ConnectionProfile>, String> {
    load_profiles(&app)
}
...
pub async fn profile_update(
    ...
    profile.engine = input.engine.clone();
    profile.label = input.label.clone();
    profile.input = input;
...
#[tauri::command]
pub async fn profile_remove(app: AppHandle, profile_id: Uuid) -> Result<(), String> {
    let mut profiles = load_profiles(&app)?;
