...
export function getDatabaseSearchResults(
  tables: TableItem[],
  schemas: string[],
  query: string
): DatabaseSearchResult[] {
  const q = query.trim().toLowerCase();
  const schemaResults: DatabaseSearchResult[] = schemas
    .filter((schema) => !q || schema.toLowerCase().includes(q))
    .map((schema) => ({ type: "schema", data: schema }));
  const tableResults: DatabaseSearchResult[] = tables
    .filter(
      (table) =>
        !q ||
...
export function DatabaseSearchDialog({
  ...
  const filteredResults = useMemo(() => {
    return getDatabaseSearchResults(tables, schemas, query);
