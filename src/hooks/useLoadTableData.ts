...
async function detectRedisKeyType(connId: string, keyName: string) {
...
async function loadRedisOverview(params: {
  connId: string;
  tableName: string;
}): Promise<{
  columns: ColumnRow[];
...
async function loadRedisRows(params: {
  key: string;
  connId: string;
  tableName: string;
  limit: number;
    ...
    async (
      ...
      const task = (async () => {
        ...
        try {
          ...
          if (activeTab.engine === "redis") {
            try {
              let redisColumns = Array.isArray(prev.columns)
                ? (prev.columns as ColumnRow[])
                : [];
              ...
              if (plan.needColumns || plan.needMeta) {
                ...
                try {
                  setColumnsCache(key, redisColumns as any);
                } catch {}
              }
              ...
              if (plan.needRows || plan.needRowCount || plan.needSizeInfo) {
                ...
                try {
                  setSizeInfoCache(key, redisSizeInfo as any);
                } catch {}
              }
